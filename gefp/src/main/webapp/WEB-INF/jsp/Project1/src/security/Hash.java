...
public class Hash {
    ...
    {
        nDatabytes = paramInt1;
        nCheckbytes = paramInt2;
        pattern = paramByte;
        k = paramInt3;
    }
    ...
    {
        return nDatabytes + nCheckbytes + 1;
    }
    ...
    {
        return pack( paramArrayOfByte, nDatabytes, nCheckbytes, pattern, k );
    }
    ...
    public byte[] pack( byte[] paramArrayOfByte, int paramInt )
    {
        ...
        return pack( arrayOfByte, nDatabytes, nCheckbytes, pattern, k );
    }

    public static byte[] pack( byte[] paramArrayOfByte, int paramInt1,
        int paramInt2, byte paramByte, int paramInt3 )
    {
        if( paramInt1 > 256 ){ throw new RuntimeException(
            "Maximum Size of databytes is 255." ); }
        ...
        for( int n = 0; n < m; n++ )
        {
            int i3 = (byte) ((n + 1) * paramInt1 > i ? i % paramInt1
                : paramInt1);

            arrayOfByte[(n * j)] = (byte) i3;
            ...
            for( int i1 = 0; i1 < i3; i1++ )
            {
                ...
                i2++;
                localBigInteger = localBigInteger.add( BigInteger.valueOf( (paramByte & b)
                    * paramInt3 ) );
                arrayOfByte[(n * j + i1 + 1)] = b;
            }

            localBigInteger = localBigInteger.mod( BigInteger.valueOf( (int) Math.pow(
                2.0D, 8 * paramInt2 ) ) );

            int b = localBigInteger.toByteArray().length;
    ...
    {
        return unpack( paramArrayOfByte, nDatabytes, nCheckbytes, pattern, k );
    }

    public static byte[] unpack( byte[] paramArrayOfByte, int paramInt1,
        int paramInt2, byte paramByte, int paramInt3 ) throws Exception
    {
        if( paramInt1 > 256 ){ throw new RuntimeException(
            "Maximum size of databytes is 256" ); }
        int i = paramArrayOfByte.length;
        int j = 1 + paramInt1 + paramInt2;
        if( i % j != 0 ){ throw new Exception( "Packet Size is wrong" ); }
        int m = i / j;
        ...
        for( ; i2 < m; i2++ )
        {
            ...
            for( int i6 = 0; i6 < i5; i6++ )
            {
                ...
                i3++;
                localBigInteger = localBigInteger.add( BigInteger.valueOf( (b & paramByte)
                    * paramInt3 ) );
                arrayOfByte[i4] = b;
                i4++;
            }
            ...
            for( int i7 = paramInt2 - b; i7 < paramInt2; i7++ )
                if( i7 >= 0 )
                {
                    ...
                    if( i8 != i9 ) throw new Exception( "wrong checksum" );
                }
            i3 += paramInt2;
        }
        return arrayOfByte;
    ...
    public static void main( String[] paramArrayOfString ) throws Exception
    {
        ...
        {
            byte[] arrayOfByte = pack( paramArrayOfString[i1].getBytes(), i, j,
                (byte) m, n );
            System.out.println( "packed Bytes" );
            System.out.println( new String( arrayOfByte ) );
            System.out.println( "unpacked Bytes" );
            System.out.println( new String( unpack( arrayOfByte, i, j,
                (byte) m, n ) ) );
        }
    }
}
