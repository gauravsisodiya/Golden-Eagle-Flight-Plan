...
public class RSA {
    ...
    public static class Key {
        ...
        public void read( InputStream paramInputStream ) throws IOException
        {
            ...
            try
            {
                key = new BigInteger( localStringBuffer.toString() );
            }
            ...
            try
            {
                n = new BigInteger( localStringBuffer.toString() );
            }
    ...
    public static class PublicKey extends RSA.Key {
        ...
        protected PublicKey( BigInteger paramBigInteger1,
            BigInteger paramBigInteger2 )
        {
    ...
    public static class PrivateKey extends RSA.Key {
        ...
        protected PrivateKey( BigInteger paramBigInteger1,
            BigInteger paramBigInteger2 )
        {
            super( paramBigInteger1, paramBigInteger2 );
        }
    ...
    public static class KeyPair {
        ...
        public KeyPair( RSA.PrivateKey paramPrivateKey,
        ...
        public String toString()
        {
            return "KR=" + kR + System.getProperty( "line.separator" ) + "KU="
                + kU;
        }
    }
    ...
    private static BigInteger relativePrime( BigInteger paramBigInteger )
    {
        ...
        } while( paramBigInteger.gcd( localBigInteger2 ).compareTo(
            localBigInteger1 ) != 0 );
        return localBigInteger2;
    }

    public static KeyPair generateKeys( BigInteger paramBigInteger1,
        BigInteger paramBigInteger2 )
    {
        BigInteger localBigInteger1 = BigInteger.ONE;
        BigInteger localBigInteger2 = paramBigInteger1.multiply( paramBigInteger2 );
        BigInteger localBigInteger3 = paramBigInteger1.subtract(
            localBigInteger1 ).multiply(
            paramBigInteger2.subtract( localBigInteger1 ) );
        ...
        return new KeyPair(
            new PrivateKey( localBigInteger5, localBigInteger2 ),
            new PublicKey( localBigInteger4, localBigInteger2 ) );
    }

    public static byte[] cipher( byte[] paramArrayOfByte, Key paramKey )
        throws Exception
    {
        ...
        byte[] arrayOfByte2 = new BigInteger( arrayOfByte1 ).modPow(
            paramKey.getKey(), paramKey.getN() ).toByteArray();

        if( arrayOfByte2[0] != 0 ){ return arrayOfByte2; }
        byte[] arrayOfByte3 = new byte[arrayOfByte2.length - 1];
        System.arraycopy( arrayOfByte2, 1, arrayOfByte3, 0,
            arrayOfByte2.length - 1 );
        return arrayOfByte3;
    }

    public static byte[] cipher( String paramString, Key paramKey )
        throws Exception
    {
    ...
    public static void main( String[] paramArrayOfString ) throws Exception
    {
        if( paramArrayOfString.length >= 1 )
        {
            ...
            if( (paramArrayOfString[0].equals( "-gen" ))
                && (paramArrayOfString.length <= 2) )
            {
                ...
                BigInteger localBigInteger1 = new BigInteger( i, j,
                    new Random() );
                BigInteger localBigInteger2 = new BigInteger( i, j,
                    new Random() );
                KeyPair localKeyPair = generateKeys( localBigInteger1,
                    localBigInteger2 );
                System.out.println( localKeyPair );
                if( paramArrayOfString.length == 2 )
                {
                    byte[] arrayOfByte1 = paramArrayOfString[1].getBytes();
                    byte[] arrayOfByte2 = cipher( arrayOfByte1,
                        localKeyPair.getPublicKey() );
                    byte[] arrayOfByte3 = cipher( arrayOfByte1,
                        localKeyPair.getPrivateKey() );
                    System.out.println( "KU(KR(M))="
                        + new String( cipher( arrayOfByte2,
                            localKeyPair.getPrivateKey() ) ) );
                    System.out.println( "KR(KU(M))="
                        + new String( cipher( arrayOfByte3,
                            localKeyPair.getPublicKey() ) ) );
                }
                return;
            }
...
/*
 * Location: D:\CSULA\MS-Thesis\CS580-Project1\ImportedClasses Qualified Name:
 * security.RSA Java Class Version: 1.1 (45.3) JD-Core Version: 0.7.1
 */
