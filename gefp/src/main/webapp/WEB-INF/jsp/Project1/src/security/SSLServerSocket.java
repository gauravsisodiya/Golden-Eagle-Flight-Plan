...
public class SSLServerSocket extends ServerSocket {
    ...
    public SSLServerSocket( int paramInt, RSA.PrivateKey paramPrivateKey,
        Properties paramProperties ) throws IOException
    {
        super( paramInt );
        sKR = paramPrivateKey;
    ...
    public SSLServerSocket( int paramInt1, int paramInt2,
        RSA.PrivateKey paramPrivateKey, Properties paramProperties )
        throws IOException
    {
    ...
    public SSLServerSocket( int paramInt1, int paramInt2,
        InetAddress paramInetAddress, RSA.PrivateKey paramPrivateKey,
        Properties paramProperties ) throws IOException
    {
    ...
    protected byte[] getGreetingToken( Socket paramSocket ) throws IOException
    {
        ...
        {
            if( k == -1 ){ throw new EOFException( "Unexpected ended Greeting" ); }
    ...
    protected Object[] handshake( Socket paramSocket ) throws Exception
    {
        ...
        String str1 = new String( RSA.cipher( arrayOfByte1, sKR ) );

        String str2 = prop.getProperty( str1 + ".public_key" );

        if( str2 == null ) throw new Exception( "Unknow User: " + str1 );
        ...
        if( !str3.equals( prop.getProperty( str1 + ".company" ) ) ){ throw new Exception(
            "Wrong company (" + str1 + ':' + str3 + ")" ); }
        int j = Integer.parseInt( prop.getProperty( str1 + ".ndatabytes" ) );
        int k = Integer.parseInt( prop.getProperty( str1 + ".ncheckbytes" ) );
        byte b = (byte) Integer.parseInt( prop.getProperty( str1 + ".pattern" ) );
        int m = Integer.parseInt( prop.getProperty( str1 + ".k" ) );
        Object[] arrayOfObject = new Object[2];

        arrayOfObject[0] = RSA.cipher( arrayOfByte3, sKR );
